...
#Simulate real time input for .wav testing
def get_realtime_audio_config(filename):
    """
    Creates an AudioConfig that throttles file reading to mimic 
    real-time speaking (16kHz * 2 bytes = 32000 bytes/sec).
    """
    class ThrottledPullStream(speechsdk.audio.PullAudioInputStreamCallback):
        ...
        def read(self, data_buffer: memoryview) -> int:
            ...
            bytes_requested = data_buffer.nbytes
            bytes_read = self._file.read(bytes_requested)
            n_bytes = len(bytes_read)
            data_buffer[:n_bytes] = bytes_read
            
            # Simulate real-time delay
            if n_bytes > 0:
                time.sleep(n_bytes / 32000) 
    ...
    stream_callback = ThrottledPullStream(filename)
    
    # 1. Corrected: pull_stream_callback
    pull_stream = speechsdk.audio.PullAudioInputStream(pull_stream_callback=stream_callback)
    
    # 2. Corrected: stream (NOT stream_input)
    return speechsdk.audio.AudioConfig(stream=pull_stream)
...
def mt_worker(mt_in_queue, tts_out_queue, target_lang, key, region):
    ...
    # Track the RAW text of the last successfully processed chunk
    last_processed_raw_text = ""

    # --- HELPER: NORMALIZE FOR ROBUST COMPARISON ---
    def normalize_text(text):
        """Strips punctuation and lowercases for loose comparison."""
        # Remove anything that isn't a word character or space
        text = re.sub(r'[^\w\s]', '', text)
        return text.lower().strip()

    def get_smart_overlap_length(s1_raw, s2_raw):
        """
        Returns the number of CHARACTERS to cut from the start of s2_raw.
        Compares normalized versions but calculates cut on original.
        """
        if not s1_raw or not s2_raw: return 0
        
        n1 = normalize_text(s1_raw)
        n2 = normalize_text(s2_raw)
        
        # Optimization: Full containment (Prefix match)
        if n2.startswith(n1):
            # This is risky: normalized lengths might differ from raw lengths due to punctuation.
            # But for ASR containment, usually safe to cut everything.
            return len(s2_raw) 
        
        # Check suffix/prefix overlap on NORMALIZED strings
        max_overlap = min(len(n1), len(n2))
        best_overlap_len_norm = 0
        
        # Look for the biggest overlap
        for i in range(max_overlap, 0, -1):
            if n1.endswith(n2[:i]):
                best_overlap_len_norm = i
                break
        
        if best_overlap_len_norm == 0:
            return 0
            
        # --- MAPPING BACK TO RAW STRING ---
        # We found an overlap of 'best_overlap_len_norm' characters in the CLEANED string.
        # We need to find how many characters that corresponds to in s2_raw.
        
        # Strategy: Count words in the overlap, remove that many words from s2_raw
        overlap_str_norm = n2[:best_overlap_len_norm]
        overlap_word_count = len(overlap_str_norm.split())
        
        # Split raw s2 into words (preserving spaces is tricky, but ASR usually gives clean spacing)
        s2_words = s2_raw.split()
        
        # Safety check
        if overlap_word_count > len(s2_words):
            return len(s2_raw)
            
        # Calculate length of the first N words in the original string
        # We rejoin the first N words to estimate the length to cut
        words_to_cut = s2_words[:overlap_word_count]
        
        # We need to account for the fact that s2_raw might have punctuation attached to these words
        # Simple heuristic: Find the position of the last word to cut
        last_cut_word = words_to_cut[-1]
        
        # Find the first occurrence of this sequence? 
        # Simpler: Just reconstruct the length roughly. 
        # Or, just use the character count of the normalized overlap as a minimum 
        # and walk forward in s2_raw until we pass that many alphanumeric chars.
        
        chars_seen_alphanum = 0
        cut_index = 0
        for char in s2_raw:
            cut_index += 1
            if char.isalnum():
                chars_seen_alphanum += 1
            
            if chars_seen_alphanum >= len(re.sub(r'[^\w]', '', overlap_str_norm)):
                break
    ...
    while True:
        ...
        if DEBUG:
            ...
            if match:
                ...
                text = match[0] # Use the clean TM text

        # 2. OVERLAP CHECK
        if last_processed_raw_text:
            # We compare the PREVIOUS FINAL TEXT vs CURRENT RAW TEXT
            cut_len = get_smart_overlap_length(last_processed_raw_text, text)
            
            if cut_len > 0:
                # Heuristic: Don't cut tiny overlaps (like "I" or "A") unless it's a perfect prefix
                if cut_len < 4 and cut_len != len(text):
                    pass
                else:
                    original = text
                    text = text[cut_len:].strip()
                    if not text:
                        print(f"   [MT SKIP] Overlap '{original[:cut_len]}...' covered entire segment.")
                    else:
                        print(f"   [MT TRIM] Cut '{original[:cut_len]}...'. New start: '{text[:15]}...'")

        # 3. UPDATE STATE
        # Save the text we are about to translate as the new "previous context"
        # If we emptied the text, we technically still "processed" that chunk of audio time,
        # but for visual continuity, we usually want to track the Match text if we found one.
        current_snapshot = match[0] if (DEBUG and match) else item["text"].strip()
        last_processed_raw_text = current_snapshot
...
def main():
    ...
    if USE_MIC: audio_config = speechsdk.audio.AudioConfig(use_default_microphone=True)
    else: audio_config = get_realtime_audio_config(AUDIO_FILE)
